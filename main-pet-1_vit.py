...
def main():
    ...
    model.heads.head = nn.Linear(num_features, 37)  # 修改为100类
    ...
    print(f"Original Oxford-IIIT Pet test set size: {len(testset)}")
...
def train(train_loader, model, criterion, optimizer, epoch, print_freq):
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        loss = ce + torch.exp(mi_bg) * 1000 + torch.exp(-align_loss) * 0.00001
