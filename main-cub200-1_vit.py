...
parser.add_argument('-a', '--arch', metavar='ARCH', default='vit_b_32', choices=model_names,
...
def main():
    ...
    # ViT的分类器层是 model.heads.head
    num_features = model.heads.head.in_features  # 获取输入特征数 (768)
    model.heads.head = nn.Linear(num_features, 200)  # 修改为100类
    model.cuda()
    ...
    class CUB200(Dataset):
        def __init__(self, root='./data', train=True, transform=None, download=False):
            ...
            with open(images_path, 'r') as f:
                self.images = [line.strip().split() for line in f]
            ...
            with open(labels_path, 'r') as f:
                self.labels = [int(line.strip().split()[1]) - 1 for line in f]

            self.data = []
            for idx, (img_id, img_path) in enumerate(self.images):
                if (train and splits[idx] == 1) or (not train and splits[idx] == 0):
                    self.data.append((img_path, self.labels[idx]))
        ...
        def download(self):
            ...
            download_and_extract_archive(url, self.root.replace('/CUB_200_2011', ''),
        ...
        def __getitem__(self, idx):
            ...
            img_full_path = os.path.join(self.root, 'images', img_path)
    ...
    print(f"Original CUB-200 test set size: {len(testset)}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            }, is_best, args.arch + '_cub200-1.pth')
